...
/// Get the mix format for an audio client via COM
#[with_com]
pub fn get_mix_format(client: ComSend<IAudioClient>) -> Result<ComSend<Vec<u8>>> {
    get_mix_format_internal(&client.take()).map(ComSend::new)
}

/// Helper that wraps client setup in COM
#[with_com]
pub fn setup_router_clients(cfg: RouterConfig) -> Result<ComSend<RouterSetupResult>> {
    setup_router_clients_internal(&cfg).map(ComSend::new)
}
...
pub fn initialize_router(
    ...
    for render_client in render_clients {
        ...
        match initialize_render_client_internal(&render_client, pwf) {
            ...
            Err(e) => eprintln!(
                "Failed to initialize render client with source format: {:?}. This is likely due to format mismatch (e.g. sample rate difference).",
                e
            ),
        }
    }
    ...
    Ok(ComSend::new(RouterInitialized {
        capture_service,
        render_services,
    }))
}
